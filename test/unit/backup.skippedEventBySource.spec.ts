...
const sampleAllowedEvent = {
	...
	"Source ID": "rds:db-dev-NOT-SUPPORTED",
...
describe("Backup", () => {
	...
	describe("GIVEN processEvent receives an event", () => {
		describe("WHEN the event message is NOT an allowed event", () => {
			...
			it("SHOULD log that the message was skipped because of not supported source", () => {
				expect(journalSpy).to.have.been.calledWithMatch(
					{
						level: "Info",
						name: "rds-s3-exporter.skipped.notsupportedsource",
						eventSource: "rds:db-dev-NOT-SUPPORTED"
					}
				)
			});
		});
